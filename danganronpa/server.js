...
app.get('/users', (req, res) => {
  ...
  db.all('SELECT first_name FROM triggerhappy', (err, rows) => {
    console.log(rows);
    const allUsernames = rows.map(e => e.first_name);
...
app.post('/users', (req, res) => {
  ...
  db.run(
    'INSERT INTO triggerhappy VALUES ($emp_num, $last_name, $first_name, $job, $status, $picture)',
...
app.get('/users/:userid', (req, res) => {
  ...
  // db.all() fetches all results from an SQL query into the 'rows' variable:
  db.all(
    'SELECT * FROM triggerhappy WHERE last_name=$last_name',
    // parameters to SQL query:
    {
      $last_name: nameToLookup
    },
    // callback function to run when the query finishes:
    (err, rows) => {
      console.log(rows);
